...
def minhash_iter(dataset_iterator: Type[Dataset]):
    # computing minhah hash of the samples in dataset iterator in parallel
...
def make_duplicate_clusters(dataset_iterator: Type[Dataset], jaccard_threshold: float):
    ...
    for filename, min_hash in tqdm(ThreadedIterator(minhash_iter(enumerate(dataset_iterator)), max_queue_size=100)):
        hashes.append((filename, min_hash))
    print(f"minhashes computed in: {time.time()-t_start:.2f}s => {(time.time()-t_start)/60:.2f}min")

    print("\nbuilding clusters")
    t_start = time.time()
    for filename, min_hash in tqdm(hashes):
        di.add(filename, min_hash)
    print(f"clusters built in: {time.time()-t_start:.2f}s => {(time.time()-t_start)/60:.2f}min")

    # Returns a List[Cluster] where Cluster is List[str] with the filenames.
    print("\nexporting the clusters")
    t_start = time.time()
    clusters = di.get_duplicate_clusters()
    print(f"clusters exported in: {time.time()-t_start:.2f}s => {(time.time()-t_start)/60:.2f}min")

    stats = [len(cluster) for cluster in clusters]
    print(f"max, min, mean and median of the cluster sizes: {max(stats)}, {min(stats)}, {np.mean(stats)}, {np.median(stats)}")
    print("saving the clusters list and stats")
    with open("./clusters_list.json", "w") as fp:
        json.dump(clusters, fp)
    with open("./clusters_stats.json", "w") as fp:
        json.dump(stats, fp)
...
def deduplicate_dataset(
    ...
    print("MinHash computation done and cluster info saved")
    ...
    print(f"Extremes found in: {time.time()-t_start:.2f}s => {(time.time()-t_start)/60:.2f}min")
    ...
    print(f"dataset filtered in: {time.time()-t_start:.2f}s => {(time.time()-t_start)/60:.2f}min")
    ...
    print(f"clusters (for analysis) updated in: {time.time()-t_start:.2f}s => {(time.time()-t_start)/60:.2f}min")
