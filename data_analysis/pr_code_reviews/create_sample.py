...
def process_bucket(bucket_id="00"):
    ...
    print(
        f"number of PRs in bucket {bucket_id}: {len(df_00['pull_request.id'].unique())}"
    )
    ...
    for name, group in tqdm(grouped_data):
        ...
        group_dict["pull_request_info"] = group[pull_request_info_cols].to_dict(
            orient="records"
        )[0]
        group_dict["head_repo_info"] = group[head_info_cols].to_dict(orient="records")[
            0
        ]
        group_dict["base_repo_info"] = group[base_info_cols].to_dict(orient="records")[
            0
        ]
